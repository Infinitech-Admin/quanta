...
export async function GET() {
  try {
    ...
    const res = await fetch(`${getApiUrl()}/api/admin/job-listings`, {
      headers: { Accept: "application/json", ...headers },
      cache: "no-store",
    });
    return await forwardJson(res);
  } catch (error) {
    console.error("GET /api/admin/job-listings error:", error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("POST /api/admin/job-listings error:", error);
    return NextResponse.json(
      { success: false, message: "Server error" },
      { status: 500 },
    );
