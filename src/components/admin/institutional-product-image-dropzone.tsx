// components/admin/institutional-product-image-dropzone.tsx
"use client";
...
interface InstitutionalProductImageDropzoneProps {
  label: string;
  helperText?: string;
  value: string | null;
...
// the auth headers attached (see
// app/api/admin/institutional-products/upload/route.ts). Don't call
// Laravel directly from the browser here — that route is behind
// auth:sanctum and the browser has no way to attach that auth itself.
const UPLOAD_ENDPOINT = "/api/admin/institutional-products/upload";
const UPLOAD_FOLDER = "institutional-products";

export function InstitutionalProductImageDropzone({
  label,
  helperText,
  value,
  onChange,
  ...
  maxSizeMb = 5,
}: InstitutionalProductImageDropzoneProps) {
  const inputId = useId();
